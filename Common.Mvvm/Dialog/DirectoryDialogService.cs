...
namespace Common.Mvvm.Dialog
{
    ...
    public class DirectoryDialogService : IDirectoryDialogService
    {
        ...
        public bool Open(string title)
        {
            ...
            var browserDirectoryDialog = new FolderPicker
            {
                InputPath = (!string.IsNullOrEmpty(SelectedDirectory)) ? SelectedDirectory : (!string.IsNullOrEmpty(InitialDirectory)) ? InitialDirectory : string.Empty,
                OkButtonLabel = "Выбрать"
            };

            var result = owner != null ? browserDirectoryDialog.ShowDialog(owner) : browserDirectoryDialog.ShowDialog();

            SelectedDirectory = browserDirectoryDialog.ResultPath;
            return result == true;
        }
    }
